...
export function Transfer() {
  ...
  const handleTransfer = async (e) => {
    ...
    try {
      const response = await api.post("/transaction/transfer", {
        ...
        receiverCpf: stripNonDigits(destinationCpf),
      });
  ...
  const handleConfirmTransfer = async (e) => {
    ...
    setErrorMsg("");
    try {
      await api.post(`/transaction/transfer/confirm/${transactionId}`, {
        password: password,
      });
  ...
  const renderConfirmationScreen = () => (
    <div className="balance-card success-card">
      ...
      <p>
        You transfer <strong>R$ {formatMoneyDisplay(amount)}</strong>{" "}
        successfully!
      </p>
      <button
